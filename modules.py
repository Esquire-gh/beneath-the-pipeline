...
def strip_state(current: Module) -> list[dict]:
    """The state of every stage in the strip, as seen from one module page.

    outlined  — not reached yet
    here      — this module builds it
    done      — an earlier module built it
    stressed  — this module is breaking it
    reinforced— an earlier Part III module already broke it
    """
    out = []
    for stage in STAGES:
        info = stage_modules(stage)
        builder = info["builder"]
        breakers = info["breakers"]

        if stage in current.builds:
            state = "here"
        elif stage in current.stresses:
            state = "stressed"
        elif builder and builder.num < current.num:
            broken_earlier = any(b.num < current.num for b in breakers)
            state = "reinforced" if broken_earlier else "done"
        else:
            state = ""

        nums = []
        if builder:
            nums.append(builder.nn)
        nums += [b.nn for b in breakers]

        out.append({
            "stage": stage,
            "state": state,
            "nums": "·".join(nums),
            "href": (builder.slug + ".html") if builder else None,
            "title": _stage_tooltip(stage, builder, breakers),
        })
    return out


def _stage_tooltip(stage, builder, breakers) -> str:
    bits = []
    if builder:
        bits.append(f"built in module {builder.num}")
    if breakers:
        bits.append("stressed in module " + ", ".join(str(b.num) for b in breakers))
    return f"{stage}() — " + "; ".join(bits) if bits else stage
